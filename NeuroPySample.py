...
sample_input = [0.56, 0.82, 0.25]
correct_answer = [0, 1]

l1_weights = npy.WeightInitializer().initNormalizedXavierWeight([6, 3], 6, 4)
...
for i in range(learn_loop):
    l1_output = npy.ForwardPropagation().createLayer(sample_input, l1_weights)
    l2_output = npy.ForwardPropagation().createLayer(l1_output, l2_weights)
    fl_output = npy.ForwardPropagation().createLayer(l2_output, fl_weights)
    ...
    l1_layer_neuron_strenght = back_propagation.getHLayerNeuronStrength(l2_output, fl_applied_weight_adjustment, fl_bp_neuron_strenght)


    l2_calculated_weight_adjustments = back_propagation.calculateWeightAdjustment(l1_layer_neuron_strenght, l1_output)
    l2_applied_weight_adjustment = back_propagation.applyWeightAdjustment(l2_weights, l2_calculated_weight_adjustments)
    l2_neuron_strenght = back_propagation.getHLayerNeuronStrength(l1_output, l2_applied_weight_adjustment, l1_layer_neuron_strenght)


    l3_calculated_weight_adjustments = back_propagation.calculateWeightAdjustment(l2_neuron_strenght, sample_input)
    l3_applied_weight_adjustment = back_propagation.applyWeightAdjustment(l1_weights, l3_calculated_weight_adjustments)
    l3_neuron_strenght = back_propagation.getHLayerNeuronStrength(sample_input, l3_applied_weight_adjustment, l2_neuron_strenght)
